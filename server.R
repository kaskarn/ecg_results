...
printqqsubgraph <- function(stud, trait, sub){
  if(sub == "") return(paste0("./graphs/",stud,"/",prefix[[stud]],"qq_",trait,".png"))
  paste0("./graphs/",stud,"/",prefix[[stud]],"qq_",trait,"_",sub,".png")
}
...
make_graphs <- function(studies, substudies, prefix, traits, mkfun){
            ...
            list(i, 
                 do.call(tabsetPanel, lapply(substudies[[i]], function (k) {
                   do.call(tabPanel, 
                           list(k,
                                fluidRow(
                                  lapply(traits, function (j) tags$img(src = mkfun(i,j,k)))
  )))}))))})
}
...
shinyServer(function(input, output) {
  output[["Studies"]] <- renderUI({
      do.call(tabsetPanel, 
              list(tabPanel("Manhattan", do.call(tabsetPanel, make_graphs(studies, substudies, prefix, traits, printmansubgraph))),
                   tabPanel("QQ", do.call(tabsetPanel, make_graphs(studies, substudies, prefix, traits, printqqsubgraph)))
))})})
